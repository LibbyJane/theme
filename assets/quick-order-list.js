...
class QuickOrderListRemoveAllButton extends HTMLElement {
    ...
    toggleConfirmation(showConfirmation, showInfo) {
        this.quickOrderList.querySelector('.js-quick-order-list-total__confirmation').classList.toggle('hidden', showConfirmation);
        this.quickOrderList.querySelector('.js-quick-order-list-total__info').classList.toggle('hidden', showInfo)
    }
}
...
class QuickOrderList extends HTMLElement {
    ...
    getSectionsToRender() {
        return [
            ...
            },
            {
                ...
                section: document.getElementById(this.quickOrderListId).dataset.id,
                selector: '.js-quick-order-list__total'
            },
            {
                id: 'CartDrawer',
                selector: '#CartDrawer',
    ...
    updateMultipleQty(items) {
        this.querySelector('.js-variant-remove-total .js-loading-overlay').classList.remove('hidden');
        ...
        fetch(`${routes.cart_update_url}`, { ...fetchConfig(), ...{ body } })
            ...
            .finally(() => {
                this.querySelector('.js-variant-remove-total .js-loading-overlay').classList.add('hidden');
            });
    }
    ...
    updateQuantity(id, quantity, name, action) {
        ...
        fetch(`${routeUrl}`, { ...fetchConfig(fetchConfigType), ...{ body } })
            ...
            .then((state) => {
                ...
                const items = document.querySelectorAll('.js-variant-item');

                if (parsedState.description || parsedState.errors) {
                    const variantItem = document.querySelector(`[id^="Variant-${id}"] .js-variant-item__totals.small-hide .js-loading-overlay`);
                ...
                this.querySelectorAll('.js-loading-overlay').forEach((overlay) => overlay.classList.add('hidden'));
    ...
    setErrorMessage(message = null) {
        ...
        const errorElements = document.querySelectorAll('.js-quick-order-list-error');

        errorElements.forEach((errorElement) => {
            errorElement.innerHTML = '';
            if (!message) return;
            const updatedMessageElement = this.errorMessageTemplate.cloneNode(true);
            updatedMessageElement.content.querySelector('.js-quick-order-list-error-message').innerText = message;
            errorElement.appendChild(updatedMessageElement.content);
        });
    }

    updateMessage(quantity = null) {
        const messages = this.querySelectorAll('.js-quick-order-list__message-text');
        const icons = this.querySelectorAll('.js-quick-order-list__message-icon');
    ...
    updateLiveRegions(id, message) {
        ...
        if (variantItemErrorDesktop) {
            variantItemErrorDesktop.querySelector('.js-variant-item__error-text').innerHTML = message;
            variantItemErrorDesktop.closest('tr').classList.remove('hidden');
        }
        if (variantItemErrorMobile) variantItemErrorMobile.querySelector('.js-variant-item__error-text').innerHTML = message;
    ...
    toggleLoading(id, enable) {
        ...
        const quickOrderListItems = this.querySelectorAll(`#Variant-${id} .js-loading-overlay`);
